...
const updatedVocabularySet = ref<IVocabSet>({
    id: String(vocabularySetStore.currentVocabularySet?.id),
    name: String(vocabularySetStore.currentVocabularySet?.name),
    description: String(vocabularySetStore.currentVocabularySet?.description),
    room: String(vocabularySetStore.currentVocabularySet?.room)
})

const submitForm = () => {
    vocabularySetStore.updateVocabularySet(updatedVocabularySet.value)
}
</script>
...
<template>
    ...
    <div data-te-modal-init
        ...
        id="updateSetModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                ...
                <div class="relative flex-auto p-4 font-semibold" data-te-modal-body-ref>
                    <form class="space-y-6" @submit.prevent="submitForm">
                        <div>
                            ...
                            <input type="text" id="name" v-model="updatedVocabularySet.name"
                        ...
                        <div>
                            ...
                            <textarea id="desc" v-model="updatedVocabularySet.description"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                        ...
                        <div class="bg-red-800 text-white p-3 rounded-md mb-3"
                            v-if="vocabularySetStore.updateVocabSetErrors.length">
                            <p v-for="error in vocabularySetStore.updateVocabSetErrors">{{ error }}</p>
                        </div>
                        <button type="submit"
