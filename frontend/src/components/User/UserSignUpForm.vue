...
const signUpUser = ref<IUserSignUp>({
    email: '',
    name: '',
    password1: '',
    password2: '',
...
const submitForm = async () => {
    signUpUser.value.name = `${firstName.value} ${lastName.value}`

    await userStore.userSignUp(signUpUser.value)

    signUpUser.value.email = ''
    signUpUser.value.name = ''
    signUpUser.value.password1 = ''
    signUpUser.value.password2 = ''
    firstName.value = ''
    lastName.value = ''
}
</script>
    ...
    <form @submit.prevent="submitForm">
        <div class="relative z-0 w-full mb-6 group">
            <input type="email" name="floating_email" id="floating_email"
                ...
                placeholder=" " required  v-model="signUpUser.email"/>
        ...
        <div class="relative z-0 w-full mb-6 group">
            <input type="password" name="floating_password" id="floating_password"
                class="block py-2.5 px-0 w-full text-sm text-gray-200 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                placeholder=" " required  v-model="signUpUser.password1"/>
        ...
        <div class="relative z-0 w-full mb-6 group">
            <input type="password" name="repeat_password" id="floating_repeat_password"
                class="block py-2.5 px-0 w-full text-sm text-gray-200 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                placeholder=" " required v-model="signUpUser.password2"/>
        ...
        <div class="grid md:grid-cols-2 md:gap-6">
            <div class="relative z-0 w-full mb-6 group">
                <input type="text" name="floating_first_name" id="floating_first_name"
                    class="block py-2.5 px-0 w-full text-sm text-gray-200 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                    placeholder=" " required v-model="firstName"/>
            ...
            <div class="relative z-0 w-full mb-6 group">
                <input type="text" name="floating_last_name" id="floating_last_name"
                    class="block py-2.5 px-0 w-full text-sm text-gray-200 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                    placeholder=" " required v-model="lastName"/>
        ...
        <div class="bg-red-800 text-white p-3 rounded-md mb-3" v-if="userStore.signUpErrors.length">
            <p v-for="error in userStore.signUpErrors">{{ error }}</p>
        </div>

        <button type="submit"
            class="text-white mb-3 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Sign Up</button>
        
        <small class="text-sm text-white md:ml-3">
            <font-awesome-icon icon="fa-solid fa-circle-info" /> Already have an account <RouterLink :to="{ name: 'sign-in' }" 
